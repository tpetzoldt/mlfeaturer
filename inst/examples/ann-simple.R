...
net <- nnet(get_x_train(dt4), get_y_train(dt4, type="both"), size=10, maxit=1000, trace=FALSE)
...
ml_evaluate(dt4, n1) # todo: check xtype and ytype

cbind(predict(net), predict(dt4, net, type="train"))

cbind(residuals(net), residuals(dt4, net, type="train"))
